...
def load_keys():
    """Load keys from file or generate new ones based on user choice"""
    try:
        choice = input("Do you want to use your own key file? (y/n): ").lower()
        
        if choice == 'y':
            # User wants to use their own key file
            filename = input("Enter the path to your key file (or just filename if in same directory): ")
            # If only filename is provided, use current directory
            ...
            filename = "3des_keys.txt"
            save_keys(key1, key2, key3, filename)
            print(f"New keys have been generated and saved to {filename}")
            return key1, key2, key3
            
    except IOError as e:
        raise IOError(f"Failed to load/generate keys: {str(e)}")
...
def unpad(padded_bin):
    ...
    :param block_length: The block length used during padding.
...
def load_plaintext():
    """Load plaintext from file with flexible path handling"""
    try:
        plaintext = ""
        filename = input("Enter the path to your text file (or just filename if in same directory): ")
        ...
        raise IOError(f"Failed to load text file: {str(e)}")

def save_text(text,process,filename="3des_result.txt"):
    try:
        with open(filename,"a") as f:
            if process=="E":
                f.write("Encrypted binary: ")
            elif process=="D":
                f.write("decrypted text: ")
            f.write(text+"\n")
        return
    except IOError:
        raise IOError("Failed to write operation result")    
...
print("This program will encrypt your text using Triple DES (3DES) algorithm.")
...
try:
    ...
    key1, key2, key3 = load_keys()
    
    # Encrypt
    print("\nEncrypting...")
    ciphertext = triple_des_encrypt(plaintext, key1, key2, key3)
    save_text(text=ciphertext, process="E")
    print("Encryption completed. Results saved to 3des_result.txt")

    # === DECRYPTION ===
    print("\n=== Decryption Process ===")
    print("Decrypting...")
    decrypted_text = triple_des_decrypt(ciphertext, key1, key2, key3)
    save_text(text=decrypted_text, process="D")
    print("Decryption completed. Results saved to 3des_result.txt")
